...
<style>
    ...
    .boo-nav-container > .boo-section.active {
        display: block;
    }
    .boo-small-padding{
        padding: 6px;
...
<main class=" ms-sm-auto p-0" id="mainWrapper">
    ...
    <div class='boo-container'>
        ...
        <div class='boo-nav-container'>
            ...
            <div class='boo-section' type='booklist'>
                <div class="boo-nav-top ">
                    <div class="uk-padding-small">
                <div class="boo-nav-control  uk-padding-small boo-creater" title='Drop item here to remove'>

                            <span uk-icon="plus"></span> Create new
                        </div>
                    </div>
                </div>
                <div class="boo-nav-section boo-medium-padding">
                <ul class="uk-grid-small uk-child-width-1-2 uk-child-width-1-1@s" uk-sortable="handle: .uk-sortable-handle" uk-grid>
                <li>
            ...
            <div class='boo-section active' type='treeview'>
                <div class="boo-nav-top uk-padding-small">
                    <h4><span class='boo-current-book-selector' uk-icon="triangle-down"></span>
                        <span>Super-book name test of long long name of the book...</span>
                    </h4>
                </div>
            ...
            <div class='boo-section' type='dayflow'>
            <div class="boo-nav-top ">
                    <div class="uk-padding-small">
                <div class="boo-nav-control  uk-padding-small boo-creater" title='Drop item here to remove'>

                            <span uk-icon="minus"></span> Do something
                        </div>
                    </div>
                </div>
                <div class="boo-nav-section">
                    here will flow
                </div>
            </div>
        </div>
...
<script>
    ...
    class BookerNav {
        constructor() {
            ...
            viewType.innerHTML = 'Books';
            viewType.setAttribute('target', 'booklist');
            viewType.classList.add('th-navbar-item');
            centralMenu.appendChild(viewType);

            let viewType1 = document.createElement('div');
            viewType1.innerHTML = 'Tree';
            viewType1.setAttribute('target', 'treeview');
            viewType1.classList.add('th-navbar-item', 'active');
            centralMenu.appendChild(viewType1);

            let viewType2 = document.createElement('div');
            viewType2.innerHTML = 'Flow';
            viewType2.setAttribute('target', 'dayflow');
            viewType2.classList.add('th-navbar-item');
            centralMenu.appendChild(viewType2);

            let buttons = document.querySelectorAll('th-navbar-item');
            document.addEventListener('click', (e) => {
                if (e.target.closest('.th-navbar-item')){
                    if (e.target.getAttribute("target") != null)
                    {
                        let val = e.target.getAttribute("target");
                        let bsc = document.querySelectorAll('.boo-section');
                        for (let index = 0; index < bsc.length; index++) {
                            const element = bsc[index];
                            element.classList.remove('active');
                            if (element.getAttribute('type') == val){
                                element.classList.add('active');
                                e.target.parentElement.querySelector('.active').classList.remove('active');
                                e.target.closest('.th-navbar-item').classList.add('active');
                            }
